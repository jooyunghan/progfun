...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    //      fs.foldRight(zero)((f, futureOfList) => async { await(f) :: await(futureOfList) } )
    //    }
    // imperative implementation - for this to work this class should not be a value class
     ...
     */
    def any[T](fs: List[Future[T]]): Future[T] = fs.foldLeft(Promise[T]())(_ tryCompleteWith _).future
